...
class SwsCyIds:
    ...
    # Not currently used
    forecasted_earnings_growth = 'report-statement-isExpectedAnnualProfitGrowthAboveMarket'
...
def _find_by_cy_id(soup: BeautifulSoup, cy_id: str, default: str = '', first_string=False) -> str:
...
def simply_wall_st(table: pd.DataFrame):
    ...
    for index, row in table.iterrows():
        ...
        try:
            ...
            if page.status_code == http.HTTPStatus.TOO_MANY_REQUESTS:
                logging.error('{}: {}'.format(page.status_code,
                                              http.HTTPStatus.TOO_MANY_REQUESTS.brief))
                break
            ...
            urls.append(sws_url)
            names.append(_find_by_cy_id(
                soup, SwsCyIds.company_title, first_string=True))
            forecasted_earnings_growth.append(_find_first(
                Patterns.forecasted_earnings_growth, _find_by_cy_id(soup, SwsCyIds.forecasted_earnings_growth)))
            forecasted_revenue_growth.append(_find_first(
                Patterns.forecasted_revenue_growth, _find_by_cy_id(soup, SwsCyIds.forecasted_revenue_growth)))
            # TODO add case for "YQ is debt free."
            debt_to_equity.append(_find_first(
                Patterns.debt_to_equity, _find_by_cy_id(soup, SwsCyIds.debt_to_equity)))
            ocf_to_debt.append(_find_first(
